...
WzSerialPort* WzSerialPort::ms_instance = nullptr;
std::mutex mutex_init;

WzSerialPort* WzSerialPort::getInstance() {
	mutex_init.lock();
	if (ms_instance == nullptr) {
		ms_instance = new WzSerialPort();
	}
	mutex_init.unlock();
	return ms_instance;
}
...
int WzSerialPort::send(const char *buf,int len)
{
	HANDLE hCom = *(HANDLE*)pHandle;
